...
class LicensePlateDetector:
    def __init__(self, source_type='image', source_path=None, camera_id=0):
        ...
        self.frame_skip = 5  # Increased frame skip for better performance
        self.buffer_size = 32
    ...
    def process_video_frame(self):
        ...
        while not self.frame_buffer.stop_event.is_set():
            try:
                frame_data = self.frame_buffer.frame_queue.get(timeout=0.1)
                ...
                self.frame_buffer.result_queue.put((frame_count, visualization, detections))
...
# [Continuing from Part 1...]
    ...
    def process_video(self):
        """Process video file or camera stream with optimized buffering"""
        if self.source_type == 'camera':
            print(f"\nOpening camera {self.camera_id}")
            cap = cv2.VideoCapture(self.camera_id)
        else:
        ...
        try:
            ...
            self.frame_buffer.stop_event.clear()
            self.processing_thread = threading.Thread(target=self.process_video_frame)
            ...
            while True:
                ...
                if frame_count % self.frame_skip == 0:
                    try:
                        self.frame_buffer.frame_queue.put((frame_count, frame), timeout=0.1)
                    except queue.Full:
                        # Skip frame if buffer is full
                        pass
                
                # Try to get and display processed frame
                try:
                    processed_data = self.frame_buffer.result_queue.get_nowait()
                    if processed_data:
                        _, visualization, detections = processed_data
                    ...
                    # If no processed frame is available, show the original frame
                    if not self.display_frame(frame):
                        break
                
                frame_count += 1
                if self.source_type == 'video' and frame_count % 30 == 0:
                    progress = (frame_count / total_frames) * 100
                    print(f"\rProgress: {progress:.1f}% ({frame_count}/{total_frames} frames)", end="")
        
        finally:
            # Cleanup
            self.frame_buffer.stop_event.set()
            self.frame_buffer.frame_queue.put(None)  # Signal processing thread to stop
...
def main():
    ...
    parser.add_argument('--buffer-size', type=int, default=32,
                      help='Frame buffer size (default: 32)')
    ...
    detector.buffer_size = args.buffer_size
